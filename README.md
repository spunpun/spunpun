...
- **Dashboard** — a "sun" gauge instead of a red bar: the sun's height and warmth encode remaining budget (high & gold when on track, low & amber when close, set & vermilion when over). Big remaining figure + % spent. Each category has its own mini-sun and bar in the same gold→amber→vermilion ramp. Design: black ground, Shippori Mincho serif + Zen Kaku Gothic New (Google Fonts; system fallbacks offline).
